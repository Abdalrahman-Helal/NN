...
def apply_training_augmentation(
    ...
    return train_dataset.map(augment, num_parallel_calls=autotune).prefetch(autotune)
...
def save_combined_history_plot(history: dict[str, list[float]], phase1_epochs: int) -> None:
    if not history["accuracy"]:
        print("No training history was created, so the combined history plot was skipped.")
        return
